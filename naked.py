...
#Pievieno NASA API key lai iegutu datus 
nasa_api_key = "u9qTAtsZzMnqQObg76eckgPQMERle6hYia5nAV64"
...
#Izvada informaciju no NASA API 
...
#Izvada NASA API iegutos datus
print("Response status code: " + str(r.status_code))
print("Response headers: " + str(r.headers))
print("Response content: " + str(r.text))
#Parbauda vai status kods ir vienads ar 200
...
#izveido tuksus masivus, kuros velak tiks ievietoti iegutie dati no NASA API
	ast_safe = []
	ast_hazardous = []
#Cik ir ieguto elementu skaits
	if 'element_count' in json_data:
		ast_count = int(json_data['element_count'])
		print("Asteroid count today: " + str(ast_count))
#Parbauda vai sodien ir kads asteroids un vai ir japilda talaka koda dala
		if ast_count > 0:
			for val in json_data['near_earth_objects'][request_date]: #Iet cauri ciklam, kas skatas, datus par asteroidu
				if 'name' and 'nasa_jpl_url' and 'estimated_diameter' and 'is_potentially_hazardous_asteroid' and 'close_approach_data' in val:
					tmp_ast_name = val['name']
					tmp_ast_nasa_jpl_url = val['nasa_jpl_url']
					if 'kilometers' in val['estimated_diameter']: #Ja ir km asterioidi, tad pieskir vertibas pie lielaka un mazaka
					...
					tmp_ast_hazardous = val['is_potentially_hazardous_asteroid'] #Ja ir bistams, tad pieskir vertibu

					if len(val['close_approach_data']) > 0: #Ja asteroids ir tuvu zemei, tad kada ir bijusi si distance un laiku, kad notika
						if 'epoch_date_close_approach' and 'relative_velocity' and 'miss_distance' in val['close_approach_data'][0]:
							tmp_ast_close_appr_ts = int(val['close_approach_data'][0]['epoch_date_close_approach']/1000)
							tmp_ast_close_appr_dt_utc = datetime.utcfromtimestamp(tmp_ast_close_appr_ts).strftime('%Y-%m-%d %H:%M:%S')
							tmp_ast_close_appr_dt = datetime.fromtimestamp(tmp_ast_close_appr_ts).strftime('%Y-%m-%d %H:%M:%S')
# Kads ir bijis atrums sim asteroidam un cik tuvu ir bijis
							if 'kilometers_per_hour' in val['close_approach_data'][0]['relative_velocity']:
								tmp_ast_speed = int(float(val['close_approach_data'][0]['relative_velocity']['kilometers_per_hour']))
							else:
								tmp_ast_speed = -1
#Ja ir bijis tuvu zemei, tad kads ir bijis sis attalums no zemes
							if 'kilometers' in val['close_approach_data'][0]['miss_distance']:
								tmp_ast_miss_dist = round(float(val['close_approach_data'][0]['miss_distance']['kilometers']), 3)
							else:
								tmp_ast_miss_dist = -1
						else: #Ja nav tadadu bijusi tad tiek pieskirtas vertibas
						...
						tmp_ast_speed = -1
						tmp_ast_miss_dist = -1
#Tiek izvaditi dati, kas tika ieguti no NASA API key
					print("------------------------------------------------------- >>")
		...
		else: #Ja nebija neviens asteroids, tad tiek izvadits pazinojums un netiek pildits iepriekseja koda dala
			print("No asteroids are going to hit earth today")
#Cik daudz ir bijusu drosu un cik daudz ir nedrosu asteroidu bijis
	print("Hazardous asteorids: " + str(len(ast_hazardous)) + " | Safe asteroids: " + str(len(ast_safe)))

	if len(ast_hazardous) > 0: #Parbauda vai ir bijusi kadi bistami asteroidi

		ast_hazardous.sort(key = lambda x: x[4], reverse=False) #Karto iegutos datus

		print("Today's possible apocalypse (asteroid impact on earth) times:") #Izvada cik daudz ir bijusu bistamu asteroidu zemes tuvuma
...
else: #Nepieklust ievaditajam NASA API key
